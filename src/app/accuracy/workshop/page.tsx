  ...
  if (workspace && snapshot) {
    return (
      <WorkshopStage
        ...
        snapshot={snapshot}
        exitHref={`/accuracy/plan?workspace_id=${encodeURIComponent(workspace.id)}`}
      />
    );
  }

  const workshopHref = workspaceId
    ? `/accuracy/workshop?workspace_id=${encodeURIComponent(workspaceId)}`
    : "/accuracy/workshop";
          ...
          {workspaces.length > 0 ? (
            <ul className="mt-2 flex flex-wrap gap-2">
              {workspaces.map((row) => (
                <li key={row.id}>
                  <Link
                    href={`/accuracy/workshop?workspace_id=${encodeURIComponent(row.id)}`}
