...
- **`tactics.json`**: Numbered tactics **1–36** from tactic detail slides (≈30–45). Populate `tactics[]` with `{ number, title, lead_function, gap_ids[], origin: "inventory" | "ideated" }`. Inventory = found in deck as planned/ongoing activity; ideated = created for high-priority gaps only (may not appear elsewhere in source).
